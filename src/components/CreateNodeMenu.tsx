...
export function CreateNodeMenu({ onCreate }: CreateNodeMenuProps) {
  ...
  const [mode, setMode] = useState<'novel' | 'sermon' | 'universal'>('universal');
  const menuRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    window.dispatchEvent(new CustomEvent('tutorial-action', { detail: { action: 'node-menu', isOpen } }));
  }, [isOpen]);
  ...
  return (
    <div className="relative" ref={menuRef} id="create-node-wrapper">
      <button 
        id="add-node-btn"
      ...
      {isOpen && (
        <div id="create-node-dropdown" className="absolute top-full left-0 mt-2 w-64 bg-[#111114] border border-[#2a2a35] rounded shadow-2xl z-[999999] flex flex-col overflow-hidden max-h-[80vh] overflow-y-auto custom-scrollbar">
          ...
          <div className="flex p-2 bg-[#1a1a1f] border-b border-[#2a2a35] gap-1">
            ...
            <button 
              ...
              className={`flex-1 py-1.5 text-xs font-bold uppercase tracking-wider rounded ${mode === 'universal' ? 'bg-[#9333ea] text-white' : 'text-gray-500 hover:text-gray-300'}`}
            >
              Universal
            </button>
          </div>
          ...
          {mode === 'novel' ? (
            <>
              <MenuItem type="master" title="Core Premise" description="The one-sentence premise everything else builds from" colorClass="text-[#fbbf24]" borderClass="hover:border-[#b45309]" />
            ...
            <>
              <MenuItem type="master" title="Big Idea" description="The one sentence every point should serve — good Anchor candidate" colorClass="text-[#fbbf24]" borderClass="hover:border-[#b45309]" />
            ...
            <>
              <MenuItem type="master" title="Main Concept" description="The overarching idea connecting your project" colorClass="text-[#fbbf24]" borderClass="hover:border-[#b45309]" />
              <MenuItem type="document" title="Document" description="Main writing canvas with full editor" colorClass="text-[#a78bfa]" borderClass="hover:border-[#4c1d95]" />
              <MenuItem type="scene" title="Section / Point" description="A smaller text block or section" colorClass="text-[#fdba74]" borderClass="hover:border-[#7c4320]" />
            </>
          )}
          ...
          {mode === 'novel' ? (
            <>
              ...
              <MenuItem type="lore" title="Lore Entry" description="Deep dives into how your world works" colorClass="text-[#2dd4bf]" borderClass="hover:border-[#134e4a]" />
              ...
              <MenuItem type="reference" title="Quick Note" description="A stray thought, not yet developed" colorClass="text-[#cbd5e1]" borderClass="hover:border-[#4a4a55]" />
            </>
          ) : mode === 'sermon' ? (
            <>
              ...
              <MenuItem type="item" title="Object Lesson" description="Physical illustrations and object lessons" colorClass="text-[#fde68a]" borderClass="hover:border-[#8c734b]" />
              ...
              <MenuItem type="reference" title="Talking Point" description="A quick application point or aside — not yet fleshed out" colorClass="text-[#cbd5e1]" borderClass="hover:border-[#4a4a55]" />
            </>
          ) : (
            <>
              <MenuItem type="character" title="Person / Entity" description="People and organizations" colorClass="text-[#67e8f9]" borderClass="hover:border-[#164e63]" />
              <MenuItem type="location" title="Location" description="Places and settings" colorClass="text-[#6ee7b7]" borderClass="hover:border-[#065f46]" />
              <MenuItem type="lore" title="Concept" description="Deep dives into topics" colorClass="text-[#2dd4bf]" borderClass="hover:border-[#134e4a]" />
              <MenuItem type="item" title="Object" description="Important items or components" colorClass="text-[#fde68a]" borderClass="hover:border-[#8c734b]" />
              <MenuItem type="quote" title="Reference" description="External references or quotes" colorClass="text-[#facc15]" borderClass="hover:border-[#854d0e]" />
              <MenuItem type="reference" title="Quick Note" description="A stray thought, not yet developed" colorClass="text-[#cbd5e1]" borderClass="hover:border-[#4a4a55]" />
            </>
          )}
          ...
          {mode === 'novel' ? (
            ...
            <>
              <MenuItem type="hub" title="Hub" description="Wire to nodes to collapse/hide them" colorClass="text-[#a855f7]" borderClass="hover:border-[#4c1d95]" />
              <MenuItem type="sequence" title="Sequence" description="Plan things sequentially" colorClass="text-[#a855f7]" borderClass="hover:border-[#4c1d95]" />
              <MenuItem type="logic" title="Logic Map" description="Map out relationships and causality" colorClass="text-[#3b82f6]" borderClass="hover:border-[#1e3a8a]" />
            </>
          )}

          <div className="text-[10px] uppercase font-bold text-gray-500 bg-[#1a1a1f] px-3 py-1.5 border-y border-[#2a2a35]">Special Mechanics</div>
          {mode === 'novel' ? (
            ...
            <MenuItem type="crucible" title="Doctrinal Tension" description="Hold two truths together, unresolved" colorClass="text-[#ef4444]" borderClass="hover:border-[#991b1b]" />
          ) : (
            <MenuItem type="crucible" title="Tension / Conflict" description="Hold two ideas together for resolution" colorClass="text-[#ef4444]" borderClass="hover:border-[#991b1b]" />
          )}

          <div className="text-[10px] uppercase font-bold text-gray-500 bg-[#1a1a1f] px-3 py-1.5 border-y border-[#2a2a35]">Universal</div>
          <MenuItem type="alias" title="Quick-Link Pin" description="A floating quick-link that bridges to a real node" colorClass="text-[#f0c050]" borderClass="hover:border-[#a88530]" />
