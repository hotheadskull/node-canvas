...
export function StoryNode({ data, selected }: NodeProps<AppNode>) {
  return (
    <>
      ...
      className={`relative p-4 bg-[#111114] border-2 rounded-xl shadow-lg transition-transform hover:-translate-y-1 ${
        selected ? 'border-[#fbbf24] shadow-[0_0_15px_rgba(251,191,36,0.3)]' : 'border-[#2a2a35]'
      }`}
    >
      <Handle type="target" position={Position.Top} className="w-3 h-3 bg-[#9333ea]" />
      ...
      <Handle type="source" position={Position.Bottom} className="w-3 h-3 bg-[#9333ea]" />
    </div>
    </>
  );
}
